...
plugins {
    ...
    // alias(libs.plugins.secrets.gradle)  // Removed for CI (no API keys needed)
}
...
// manifest.json removed for Aurora fork

android {
    namespace = "app.gamenative"
    compileSdk = 35  // Changed from 36 (not available in CI yet)
    ...
    buildTypes {
        ...
        // release-signed and release-gold removed (need keystore)
    }
    ...
    // Configure Assets - simplified for Aurora (use main assets only)
    sourceSets {
        getByName("legacy") {
