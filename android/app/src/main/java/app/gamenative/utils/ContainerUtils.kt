...
object ContainerUtils {
    ...
    fun getOrCreateContainer(context: Context, appId: String): Container {
        ...
        // Aurora Phase 1: AOT Texture Transcoding
        // Scan for .ktx2 files in the game directory and transcode to .astc
        // (Only runs if the native aurora_texture library is available)
        if (gameFolderPath != null) {
            try {
                val transcoded = AuroraTextureHelper.processTextures(context, gameFolderPath)
                if (transcoded > 0) {
                    Timber.i("Aurora: transcoded $transcoded textures to ASTC")
                }
            } catch (e: Exception) {
                Timber.w(e, "Aurora texture processing failed (non-fatal)")
            }

            // Aurora Phase 2: AOT Mesh Simplification
            // Scan for .obj files and simplify at multiple LOD levels via QEM
            try {
                val meshCount = AuroraMeshHelper.processMeshes(context, gameFolderPath)
                if (meshCount > 0) {
                    Timber.i("Aurora: simplified $meshCount meshes to multiple LODs")
                }
            } catch (e: Exception) {
                Timber.w(e, "Aurora mesh processing failed (non-fatal)")
            }
        }
